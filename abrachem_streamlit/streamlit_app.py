...
with st.sidebar:
    ...
    if store.backend_activo() == "supabase":
        st.success("Supabase conectado ✓", icon="🟢")
    else:
        st.error("Supabase no está conectado.", icon="🔴")
        motivo = store.motivo_fallback()
        if motivo:
            st.caption(motivo[:350])
...
with tab_res:
    try:
        ...
        st.error("No se pudieron leer los resultados desde Supabase.")
        st.exception(e)
        filas = []
